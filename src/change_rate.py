...
"""
    2、电压差变化速率=电压差扩大幅度（按10天为一个周期）
    @brief:day volt_diff change rate  
    @zhaoCQ 2020.01.08
"""
...
class Change_rate:
    ...
    def __vol_diff(self,df,K):        # call by  filter data  ,need modify 
        """
        param:   df:data;  K: exclude top K and bottom K
        return: difference voltage
        only in
        ...
        for index in range(view_rows): 
            ...
            t=row_data_sort[len(row_data_sort)-1-K]-row_data_sort[K]        # exclude top_k bottom_k voltage differece 
            if t>abnormal_diff_big_value:
                abnormal_value_count=abnormal_value_count+1
                #print("too big row index is %d" %(index))
            
            else:
                statistic_rows=statistic_rows+1
                voltage_diff.append(t) 
    ...
    def  change_rate(self,filename,day_peroid,k): 
        """
        @param:  day_peroid: days internal,  k:  exclude min max  k number value  
        ...
        for i in range(len(mean_day_volt_diff)-1,1,-1): 
...
filename_static='/home/zhao/python/data_statistic/static.csv'
filename_slow_charge='/home/zhao/python/data_statistic/slow_charge.csv'  
